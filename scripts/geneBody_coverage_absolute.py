#!/media/root/ocean/shared/bin/miniconda2/bin/python
'''
	...
	for chrom, strand, positions in position_list.values():
		...
		for pileupcolumn in samfile.pileup(chrom, chrom_start, chrom_end, truncate=True):
			...
			for pileupread in pileupcolumn.pileups:
				...
				if not pileupread.is_head:continue
...
def Rcode_write(dataset,file_prefix, format='pdf', length=100):
	...
	if len(names) == 1:
		print >>ROUT, "plot(x,%s,type='l',xlab=\"Gene body, bp  (5\'->3\')\", ylab=\"Starts coverage\",lwd=0.8,col=icolor[1])" % (names[0])
...
def main():
	...
	Rcode_write(dataset, options.output_prefix + '.geneBodyCoverage', format = options.output_format,length=options.min_mRNA_length)
